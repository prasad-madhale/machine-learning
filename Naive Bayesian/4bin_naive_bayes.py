...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 18:38:48 2019
...
def train_naive_bayes(train_data, train_labels, test_data, test_labels, error_table, preds, labels):
    ...
    non_spam_mean = np.mean(np.take(train_labels, non_spam_indices, axis = 0))
    spam_mean = np.mean(np.take(train_labels, spam_indices, axis = 0))
    
    train_data = preprocess(train_data, mean, mins, maxs, non_spam_mean, spam_mean)
    ...
    test_data = preprocess(test_data, mean, mins, maxs, non_spam_mean, spam_mean)
    ...
    accs.append(acc(predictions, test_labels))
...
def preprocess(data, mean, mins, maxs,non_spam_mean, spam_mean):
    
    for x in data:
        for f in range(len(x)):
            if non_spam_mean < mean[f]:
                low_mean = non_spam_mean
                high_mean = spam_mean
            else:
                low_mean = spam_mean
                high_mean = non_spam_mean
            
            if x[f] >= mins[f] and x[f] <= low_mean:
                x[f] = 0
            elif x[f] > low_mean and x[f] <= mean[f]:
                x[f] = 1
            elif x[f] > mean[f] and x[f] <= high_mean:
                x[f] = 2
            elif x[f] > high_mean and x[f] <= maxs[f]:
                x[f] = 3
