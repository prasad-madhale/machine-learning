...
def cost(data, labels, weights): 
    ...
    '''
    round_vect = np.vectorize(rounder)
    predictions = np.dot(data, weights)
    return np.sum((round_vect(predictions) - labels)**2) / len(data)


def train(train_data, learn_rate = 0.0001, max_iter = 550):
    '''
    Args
    ...
    for itr in range(max_iter):   
        ...
        # vectorize rounder operation to apply on each element of predict
        round_vect = np.vectorize(rounder)  
        predicts = round_vect(predicts)
